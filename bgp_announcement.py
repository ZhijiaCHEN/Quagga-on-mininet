...
class SimpleTopo(Topo):
    def __init__(self):
        ...
        BGP_speaker = self.addSwitch('BGP_speaker')  # a node that will announce routes using ExaBGP
        Quagga = self.addSwitch('Quagga')  # a node running quagga routing suite
        R = self.addSwitch('R')  # a node with ip_forward enabled
        h1 = self.addNode('h1')
        h2 = self.addNode('h2')
        self.addLink(BGP_speaker, Quagga)
...
def main():
    ...
    for s in net.switches:
        if s.name == "BGP_speaker":  # configure the BGP speaker
            ...
            s.cmd("/usr/lib/quagga/zebra -f conf/zebra-{0}.conf -d -i /tmp/zebra-%s.pid > logs/{0}-zebra-stdout 2>&1".format(s.name))
            s.waitOutput()
            s.cmd("/usr/lib/quagga/bgpd -f conf/bgpd-{0}.conf -d -i /tmp/bgp-{0}.pid > logs/{0}-bgpd-stdout 2>&1".format(s.name), shell=True)
            s.waitOutput()
